...
feature_extractor_emo = AutoFeatureExtractor.from_pretrained("xmj2002/hubert-base-ch-speech-emotion-recognition")
model_emo = AutoModelForAudioClassification.from_pretrained("xmj2002/hubert-base-ch-speech-emotion-recognition")\
              .to(DEVICE).eval()
if torch.cuda.device_count() > 1:
    ...
    for signals, recs in tqdm(loader, desc="Processing"):
        ...
        for rec, age, gid, eid in zip(recs, ages, gids, eids):
            ...
            if txt:
                ...
                seg = "".join(ch+"。" if lab in ("E","S") else ch for ch,lab in zip(txt, labs))
